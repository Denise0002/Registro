...
db = orm.SQLiteORM("Registro alumnos")
db.crear_tabla("Registro_alumno")
#funcion limpiar
...
def nuevo(ventana):
    ...
    db.insertarUno('registro',date)
    showinfo(title='save', message='nuevo registro agregado')
    #nuevo
    id = db.mostrar('Registro', where=f'Registro="{Alumnos}"')[0][0]
    print(id)
    ventana.tabla_datos.insert('',END, text=id, values=(Alumnos,fecha,descripcion))
    limpiar(ventana)
#eliminar 
def eliminar(ventana):
    ...
    db.eliminar('registro',where=f'id= "{dato}"')
